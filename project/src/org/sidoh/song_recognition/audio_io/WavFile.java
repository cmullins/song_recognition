...
public class WavFile
{
	...
	private double floatScale = 10000d;				// Scaling factor used for int <-> float conversion				
...
//			wavFile.floatScale = 0.5 * ((1 << wavFile.validBits) - 1);
	...
	public static void copyInto(float[] a, double[] b) {
	...
	public int readFrames(float[] _sampleBuffer, int offset, int numFramesToRead) throws IOException, WavFileException
	{
		double[] sampleBuffer = new double[_sampleBuffer.length];//fatda(_sampleBuffer);
		if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");
		...
		copyInto(_sampleBuffer, sampleBuffer);
